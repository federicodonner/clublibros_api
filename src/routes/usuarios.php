...
$app->get('/api/usuarios', function (Request $request, Response $response) {
  ...
  // Verify if a company is selected
  $empresa = $request->getQueryParams()['empresa'];
  // If its selected, return only the users from that company
  if($empresa){
    $sql = "SELECT * FROM usuarios WHERE empresa = $empresa AND activo = 1";
  }else{
